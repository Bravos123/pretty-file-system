var prettyFiles = (function(){





var folderRenderer, itemDeleter, moveItem, menuOptions, dragItemGhost, interaction, itemCreation, inputPrompt, helpFunctions, createFolder;
folderRenderer = function () {
  ...
  function initModule(interact) {
    this.moveItem = function (itemToMove, target, interact) {
      ...
      for (var i = 0; i < interact.sharedResourceFolderJs.length; i++) {
        ...
        executeMoving(interact.sharedResourceFolderJs[i].getHashes()[itemToMove.id], targetContainer);
      }
    };
    ...
    function itemDown(item) {
      ...
      for (var i = 0; i < itemChilds.length; i++) {
        if (itemChilds[i] == item.element) {
          insertBefore = i + 1;
          if (insertBefore > itemChilds.length - 1) {
            insertBefore = itemChilds.length - 1;
          }
          break;
        }
      }
...
menuOptions = function (itemDeleter, moveItem) {
  function initModule(outsideFunctions, messageHandler, interact, rootContent) {
    ...
    function outsideClick(e) {
      ...
      if (!isButtonClick && e.target != menu) {
        //Clicked outside
        removeMenu();
      }
    }
...
interaction = function (menuOptionsModule, moveItem, dragItemGhost) {
  ...
  return { init: initModule };
}(menuOptions, moveItem, dragItemGhost);
itemCreation = function (folderRenderer, interaction) {
  function initModule(rootContent, messageHandler, outsideFunctions, interact) {
    ...
    */
    ...
    function createFolder(folderName, createdByUser) {
      ...
      if (interact.selectedItem.content == undefined) {
        //We have selected a file, take it's parent folder
        ...
        //We have selected a folder, use it
      ...
      for (var i = 0; i < interact.sharedResourceFolderJs.length; i++) {
        if (newFolder.parent.root != undefined) {
          //Select root as target folder
          interact.sharedResourceFolderJs[i].selectItemFromReference(undefined);
        } else {
          //Select mirrored parent folder as target
          interact.sharedResourceFolderJs[i].selectItemFromReference(interact.sharedResourceFolderJs[i].getHashes()[newFolder.parent.id]);
        }
    ...
    function createItem(fileName, onCreate) {
      ...
      for (var i = 0; i < interact.sharedResourceFolderJs.length; i++) {
        if (newFile.parent.root != undefined) {
          //Select root as target folder
          interact.sharedResourceFolderJs[i].selectItemFromReference(undefined);
        } else {
          //Select mirrored parent folder as target
          interact.sharedResourceFolderJs[i].selectItemFromReference(interact.sharedResourceFolderJs[i].getHashes()[newFile.parent.id]);
        }
      ...
      interact.newlyCreatedFile = newFile;  //return newFile;
    }
    function checkFileNameAlreayExists(contentArr, targetName) {
      if (interact.params.onlyUniqueNames) {
        for (var i = 0; i < interact.namesOfFiles.length; i++) {
...
createFolder = function (itemCreation, inputPrompt, helpFunctions) {
  function initModule() {
    var rootContent = {
        root: true,
        content: [],
    ...
    var outsideFunctions = {
        ...
        setSharedResources: function (folderJsObjectToMirror) {
          interact.sharedResourceFolderJs.push(folderJsObjectToMirror);  //folderJsObjectToMirror.setContent(rootContent);
        },
        getRoot: function () {
          return rootContent;
        },
    ...
    var itemCreate = new itemCreation.init(rootContent, messageHandler, outsideFunctions, interact);
